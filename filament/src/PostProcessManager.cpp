...
FrameGraphId<FrameGraphTexture> PostProcessManager::generateMipmapSSR(FrameGraph& fg,
     ...
     *  sigma0, so we get:
     *      sigma = sigma0 * 2.23^lod / 2.23
     *      lod   = log2(2 * sigma / sigma0) / log2(2.23) = [ log2(sigma) + log2(2 / sigma0) ] * 0.8614
     *
     *      +----------------------------------------------------+
     *      |  lod = [ log2(sigma) + log2(2 / sigma0) ] * 0.8614 |
     *      +----------------------------------------------------+
     *
     ...
     *
     *      lod   = [ log2(sigma)       + log2(      2 / (s * sigma0)) ] * 0.8614
     *      lod   = [ log2(a / sqrt(2)) + log2(      2 / (s * sigma0)) ] * 0.8614
     *      lod   = [ log2(a)           + log2(sqrt(2) / (s * sigma0)) ] * 0.8614
     *
     *   +---------------------------------------------------------------------------------------+
     *   | lod   = [ log2(a / d) + log2(sqrt(2) / ((tan(fov) / resolution) * sigma0)) ] * 0.8614 |
     *   +---------------------------------------------------------------------------------------+
     */

    const float refractionLodOffset = std::log2(f::SQRT2 / (texelSizeAtOneMeter * sigma0));
